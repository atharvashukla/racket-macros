...
(define-syntax apply-first-order
  (syntax-parser
    ...
    [(_ func:id args ...)
     ;; using `match-define`, definesbinds the names
     ;; to the extracted value of func
     (match-define (first-order arg-length temp-func)
       ;; `syntax-local-value` extracts the struct from the context
  ...
  (syntax-parser ;; <- produces a matching proc of 1 arg
    ...
    [(_ (func args ...) body ...)
     ...
     #'(begin
         ...
         ;; every name of the function has been turned into
         ;; a struct containing arg-length and  copy
         (define-syntax func
...
![](https://atharvashukla.com/files/pictures/first-order2-pic.png)
